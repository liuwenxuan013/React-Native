...
async function startReactNativeServerAsync(projectRoot, options = {}, verbose = true) {
  ...
  const customLogReporterPath = require.resolve(_path().default.join(__dirname, 'reporter'));

  let packagerOpts = {
    port: packagerPort,
    customLogReporterPath,
    // TODO: Bacon: Support .mjs (short-lived JS modules extension that some packages use)
    sourceExts: (0, _paths().getManagedExtensions)([], {
      isTS: true,
      isReact: true,
      isModern: false
    })
...
async function startExpoServerAsync(projectRoot) {
  ...
  const manifestHandler = async (req, res) => {
    try {
      ...
      let {
        exp: manifest
      } = (0, _config().readConfigJson)(projectRoot); // Get packager opts and then copy into bundleUrlPackagerOpts
